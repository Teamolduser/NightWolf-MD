...
conn.sendButton(m.chat, minesweeper.generate_string(empty)+ '\n' + 'if you win you can get *exp🧬*', wm, null, [['Giveup🖐️', usedPrefix + 'mw nyerah']], m)
...
return conn.sendButton(m.chat, `You GiveUp❕\nAnda menyerah❕\n\nExp Anda dikurangi *10 exp🧬* / your exp minus *10 exp🧬*`, wm, null, [['Ok', 'ok'], ['Play Again', usedPrefix + 'mw start']], m)
...
if(F){
                ...
                g.current[oY - 1][oX - 1] = g.map[oX - 1][oY - 1]
                    if(g.map[oY - 1][oX - 1] === 0){
                        ...
                        conn.sendButton(m.chat, 'YOU WIN🎊🎉\n\n *🎁Hadiah/gift:* 9999Exp🧬', wm, null, [['Play Again', usedPrefix + 'mw start']], m)
                        mines[m.chat] = {}
                        game = false
                        
			global.db.data.users[m.sender].exp -= 100
                      return conn.sendButton(m.chat, await minesweeper.generate_string(g.map) + '\n' + '*You Lose* Exp -100', wm, null, [['Again', usedPrefix + 'mw start']], m)
                    }
                }
