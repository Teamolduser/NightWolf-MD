...
handler.before = async function (m) {
  let id = m.chat
  if (!m.quoted || !m.quoted.fromMe || !m.quoted.isBaileys || !/Ketik.*nime/i.test(m.quoted.contentText)) return !0
  ...
  if (m.quoted.id == this.tebakanime[id][0].id) {
    ...
    if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
      ...
      await this.sendBut(m.chat, `*Benar!* +${this.tebakanime[id][2]} XP\n+1 Tiketcoin`, wm, 'Tebak Anime', '.tebakanime')
