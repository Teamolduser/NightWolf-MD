...
handler.before = async function (m) {
    let id = m.chat
    if (!m.quoted || !m.quoted.fromMe || !m.quoted.isBaileys || !/Ketik.*suska/i.test(m.quoted.contentText)) return !0
    ...
    if (m.quoted.id == this.susunkata[id][0].id) {
        ...
        if (['.suska','Bantuan',''].includes(m.text)) return !0
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            global.db.data.users[m.sender].exp += this.susunkata[id][2]
            global.db.data.users[m.sender].tiketcoin += 1
            //m.reply(`*Benar!*\n+${this.susunkata[id][2]} XP\n+1 Tiketcoin`)
            await this.sendBut(m.chat, `*Benar!* +${this.susunkata[id][2]} XP\n+1 Tiketcoin`, wm, 'Susun Kata', '.susunkata')
