...
handler.before = async function (m) {
    let id = m.chat
    if (!m.quoted || !m.quoted.fromMe || !m.quoted.isBaileys || !/Ketik.*ao/i.test(m.quoted.contetText)) return !0
    ...
    if (m.quoted.id == this.asahotak[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            ...
            await this.sendBut(m.chat, `*Benar!* +${this.asahotak[id][2]} XP\n+1 TiketCoin`, wm, 'Asah Otak', '.asahotak', m)
