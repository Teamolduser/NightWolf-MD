...
handler.before = async function (m) {
    let id = m.chat
    if (!m.quoted || !m.quoted.fromMe || !m.quoted.isBaileys || !/Ketik.*leka/i.test(m.quoted.contentText)) return !0
    ...
    if (m.quoted.id == this.lengkapikalimat[id][0].id) {
        ...
        if (['.leka','Bantuan',''].includes(m.text)) return !0
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            global.db.data.users[m.sender].exp += this.lengkapikalimat[id][2]
            //m.reply(`*Benar!*\n+${this.lengkapikalimat[id][2]} XP`)
            await this.sendBut(m.chat, `*Benar!* +${this.lengkapikalimat[id][2]} XP`, wm, 'Lengkapi Kalimat', '.lengkapikalimat', m)
